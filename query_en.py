...
API_ID = '9346538' 
API_HASH = '2f7daac678672e08fa9a3338082a1008'
...
class PixelTod:
    ...
    def main(self):
        action = int(input(f'{Fore.LIGHTBLUE_EX}Select an action:\n{Fore.LIGHTWHITE_EX}1. Start creating querry\n{Fore.LIGHTWHITE_EX}2. Create session\n>'))
        ...
        if action == 1:
            ...
            if not accounts:
                raise ValueError(f"{Fore.LIGHTRED_EX}Create session")
            ref = input(f"{Fore.LIGHTYELLOW_EX}Specify the ref:\n")
            self.ref = ref
            peer = input(f"{Fore.LIGHTYELLOW_EX}Specify the name of the bot:\n")
            self.peer = peer
            name = input(f"{Fore.LIGHTYELLOW_EX}Please provide a short name:\n")
            self.name = name
            while True:
    ...
    def pars_sessions(self):
        ...
        self.log(f"{Fore.LIGHTYELLOW_EX}Sessions found: {Fore.LIGHTWHITE_EX}{len(sessions)}!")
        return sessions
    def create_sessions(self):
        while True:
            session_name = input(F'{Fore.LIGHTBLUE_EX}Enter the session name (to exit, press Enter)\n')
            if not session_name:
                return
            ...
            if USE_PROXY:
                ...
                if session_name in proxy_dict:
                    ...
                    self.log(f'{Fore.LIGHTYELLOW_EX}Session added +{user_data.phone_number} @{user_data.username} PROXY {proxy.split(":")[0]}')
                else:
                    with pyrogram.Client(
                        api_id=API_ID,
                        api_hash=API_HASH,
                    ...
                    self.log(f'{Fore.LIGHTYELLOW_EX}Session added +{user_data.phone_number} @{user_data.username} PROXY : NONE')
            else:
                with pyrogram.Client(
                        api_id=API_ID,
                        api_hash=API_HASH,
                ...
                self.log(f'{Fore.LIGHTYELLOW_EX}yhu +{user_data.phone_number} @{user_data.username} PROXY : NONE')

    def check_valid_sessions(self, sessions: list):
        self.log(f"{Fore.LIGHTYELLOW_EX}Checking sessions for validity!")
        valid_sessions = []
        if USE_PROXY:
            proxy_dict = {}
            with open('proxy.txt', 'r') as file:
            ...
            for session in sessions:
                try:
                    ...
                    self.log(f"{Fore.LIGHTRED_EX}{session}.session is invalid")
            self.log(f"{Fore.LIGHTYELLOW_EX}Valid sessions: {Fore.LIGHTWHITE_EX}{len(valid_sessions)}; {Fore.LIGHTYELLOW_EX}Invalid: {Fore.LIGHTWHITE_EX}{len(sessions) - len(valid_sessions)}")
